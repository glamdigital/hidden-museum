...
/** Configure a HTTP proxy for the synchronization data transfers, e.g. a Content
 * Delivery Network (CDN).
 *
 * This proxy is supposed to operate in reverse proxy mode which means that when
 * it is configured all the synchronization requests are directly made on this
 * host which in turn requests Moodstocks' backend when appropriate.
 *
 * The only supported protocol is HTTP on port 80.
 *
 * This is an advanced feature that requires a specific setup. Please contact
 * our support for more information. OTHERWISE: DO NOT USE IT!
 *
 * @param host the proxy host name, e.g. "cdn.example.com".
 * @param error the pointer to the error object, if any.
 * @return `YES` if it succeeded, `NO` otherwise.
 *
 * @warning **IMPORTANT**: you must call this function *before* opening the scanner. Any
 * attempt to change this proxy setting on an opened scanner will result in a misuse error.
 * If you need to change the settings later on, close the scanner, call this function, then
 * re-open the scanner.
 *
 * If you need to zero out this proxy settings (i.e no more use a sync proxy),
 * close the scanner and call this function with a `nil` host.
 */
- (BOOL)setSyncProxy:(NSString *)host
               error:(NSError **)error;
