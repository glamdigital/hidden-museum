 ...
 *
 * - if an offline image recognition result or barcode decoding result has been previously found, it has been cached and
 * will be returned immediately through the [MSManualScannerSessionDelegate session:didFindResult:optionalQuery:] method,
 * - if no result has been cached on the client-side, a server-side search is launched. This results in the
 * [MSManualScannerSessionDelegate sessionWillStartServerRequest:] being notified right before the API search launches,
 * followed once the server request ends by a call to [MSManualScannerSessionDelegate session:didFindResult:optionalQuery:]
 * with either `nil` as result if the server found no result, or the found `MSResult`,
 * - if any error occurs in this process, the [MSManualScannerSessionDelegate session:didFailWithError:] method gets notified
 * of the responsible error instead of the [MSManualScannerSessionDelegate session:didFindResult:optionalQuery:] method.
 * - if [MSManualScannerSession cancel] is called, neither [MSManualScannerSessionDelegate session:didFindResult:optionalQuery:]
 * nor [MSManualScannerSessionDelegate session:didFailWithError:] gets called.
 *
